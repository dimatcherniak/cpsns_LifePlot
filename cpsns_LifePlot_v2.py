    ...
    while True:
        ...
        if bNeedToRedraw:
            ...
            for key, val in myDict.items():
                ...
                data = None
                while not val[5].empty():
                    if data is None:
                        data = val[5].get()
                    else:
                        data = np.hstack((data, val[5].get()))
                ...
                if val[6] is None:
                    Fs = val[2]
                    lng = 1000
                    timeAxis = np.linspace(0, lng * nPackSize / Fs, lng * nPackSize)
                    line, = ax.plot(timeAxis, np.zeros(lng * nPackSize), label=str(key))
